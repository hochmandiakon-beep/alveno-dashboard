...
const server = http.createServer(async (req, res) => {
  ...
  // API: AI Chat proxy (Anthropic Claude)
  if (p === "/api/chat" && req.method === "POST") {
    const ANTHROPIC_KEY = process.env.ANTHROPIC_API_KEY || "";
    if (!ANTHROPIC_KEY) {
      res.writeHead(400, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: "ANTHROPIC_API_KEY není nastaven. Přidejte ho v Render → Environment." }));
      return;
    }
    let body = "";
    ...
    try {
      const result = await new Promise((resolve, reject) => {
        ...
        r2.setTimeout(30000, () => { r2.destroy(); reject(new Error("AI Timeout")); });
        r2.write(body);
        r2.end();
      });
      res.writeHead(result.status, { "Content-Type": "application/json; charset=utf-8" });
      ...
      res.writeHead(502, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: "AI chyba: " + e.message }));
    }
    return;
  }
